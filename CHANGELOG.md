...
### Fixed
- `get_local_llm_config` was reading from the env-prefixed `/blitzlog/<env>/users/<login>/local-llm` path while the user-pool Terraform writes (and the README documents) the env-independent `/blitzlog/users/<login>/local-llm`. With the buggy path, the lookup raised `ParameterNotFound` and the function silently returned `None` — no log line, no failure signal, and the bootstrap rendered cloud-mode despite per-user local LLM config being present. Now reads via `BOT_POOL_SSM_PATH`, matching `list_bot_pool` and `get_telegram_user_id`.
