#!/bin/bash
# scripts-docker/02-systemd.sh — Amazon Linux 2023 systemd setup.
#
# Runs after image-baking (which started and stopped dockerd manually). Enables
# docker.service so it starts at boot, and writes the host-side watchdog +
# load-image helpers under /usr/local/bin/.
...
# /usr/local/bin/load-image.sh — loads the baked container image into docker.
cat > /usr/local/bin/load-image.sh <<'LOAD_IMAGE_EOF'
#!/bin/bash
# Loads the blitzlog-agent image baked into the AMI into the local docker store.
set -euo pipefail
...
# /usr/local/bin/watchdog.sh — autonomous-mode orchestrator.
# Polls IMDS every 5s for spot interruption; sends SIGTERM to the container
# via `docker stop --time=120` when detected. Otherwise waits for the
# container to exit (opencode run --agent build) and uploads logs.
...
# Load the baked image into docker.
for img in /opt/blitzlog/images/*.tar.gz; do
    [ -e "$img" ] || continue
    docker load -i "$img"
done

# Pull the compose file (or use the baked-in fallback). With a single
# image and no compose, this just sets the agent tag the container uses.
AGENT_TAG="${AGENT_TAG:-2.0.0}"

# Start the agent container in the background. Container writes to /workspace
# (mounted from the host) for the session export; host uploads at exit.
...
# Poll IMDS for spot interruption notices. 5s cadence, IMDSv2 only.
...
# Upload logs and any session export the container wrote to /workspace.
...
# Terminate instance (IMDSv2; tag-conditioned by the agent policy).
