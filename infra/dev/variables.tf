...
variable "spot_instance_types" {
  description = "EC2 spot instance types the dev Lambda will try, in preference order. Forwarded to the Lambda as the SPOT_INSTANCE_TYPES_JSON env var so operators can switch families per region without rebuilding the Lambda zip. Override in terraform.tfvars for regions without Arm (t4g.*) spot inventory; defaults match the core module."
  type        = list(string)
